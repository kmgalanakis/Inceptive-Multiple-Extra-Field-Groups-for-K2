...
class K2ModelItem extends K2Model
{
	...
	function getItemExtraFields($itemExtraFields, &$item = null)
	{
		...
		$query = "SELECT exfgID FROM #__k2_multiple_extra_field_groups WHERE catID=".(int)$item->catid;
		$db->setQuery($query);
		...
		$query = "SELECT ef.*, mefg.catID, mefg.exfgID, efg.name FROM #__k2_extra_fields ef 
